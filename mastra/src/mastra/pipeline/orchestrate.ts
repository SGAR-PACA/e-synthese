...
export async function runRagCore(args: {
  ...
  const config = args.config ?? await getConfig();
  const plan = await planifier(args.question, args.planner, config.maxSearchQueries);
  ...
  const { chunks } = await rechercherMultiple(plan.requetes, args.question, args.allowedCollections, config);
